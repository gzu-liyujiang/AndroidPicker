...
public class WheelView extends View {
    private final int CENTER_WHEEL_BACKGROUND = R.drawable.wheel_picker_line;
    /**
     * Scrolling duration
     */
    ...
    private void initData(Context context) {
        ...
        LABEL_OFFSET = Common.toPx(context, LABEL_OFFSET_DIP);
    ...
    private void initResourcesIfNecessary() {
        ...
        if (centerDrawable == null) {
            centerDrawable = Common.getDrawable(getContext(), CENTER_WHEEL_BACKGROUND);
        }
    ...
    private static class AnimationHandler extends Handler {
        ...
        public AnimationHandler(WheelView wheelView) {
            this.weakReference = new WeakReference<WheelView>(wheelView);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            int currY = wheelView.scroller.getCurrY();
            int delta = wheelView.lastScrollY - currY;
            ...
            if (Math.abs(currY - wheelView.scroller.getFinalY()) < MIN_DELTA_FOR_SCROLLING) {
                currY = wheelView.scroller.getFinalY();
                wheelView.scroller.forceFinished(true);
            }
            if (!wheelView.scroller.isFinished()) {
                sendEmptyMessage(msg.what);
            } else if (msg.what == wheelView.MESSAGE_SCROLL) {
                wheelView.justify();
            } else {
    ...
    public static class WheelArrayAdapter<T> implements WheelAdapter {
         ...
         */
        public WheelArrayAdapter(ArrayList<T> items, int length) {
            this.items = items;
            this.length = length;
        }
    ...
    public static class WheelNumericAdapter implements WheelAdapter {
         ...
         */
        public WheelNumericAdapter(int minValue, int maxValue) {
         ...
         */
        public WheelNumericAdapter(int minValue, int maxValue, String format) {
