...
/**
 * 基于源码定制日期时间选择器
 * 参见{@link DateTimePicker}、{@link DatePicker}和{@link TimePicker}
 */
public class DateTimeCustomPicker extends WheelPicker {
    /**
     * 不显示
     */
    ...
    public DateTimeCustomPicker(Activity activity) {
        this(activity, YEAR_MONTH_DAY, NONE);
        setUseWeight(true);
        setDividerRatio(WheelView.DividerConfig.FILL);
        setLabel("", "", "", "", "");
    }

    private DateTimeCustomPicker(Activity activity, @DateMode int dateMode, @TimeMode int timeMode) {
        super(activity);
